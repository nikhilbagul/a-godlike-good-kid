...
public class cameraMasking : MonoBehaviour
{
    ...
    private void EraseBrush(Vector2 imageSize, Vector2 imageLocalPosition)
    {
        ...
        Rect positionRect = new Rect(
            ...
            1.5f*EraserMaterial.mainTexture.width / imageSize.x,
            1.5f*EraserMaterial.mainTexture.height / imageSize.y
        ...
        for (int i = 0; i < EraserMaterial.passCount; i++)
        {
            ...
            GL.End();
			wipedArea.size += EraserMaterial.mainTexture.texelSize;
    ...
    public IEnumerator Start()
    {
        ...
        //Set Mask Texture to dust material to Generat/2e Dust erase effect
    ...
    public void Update()
    {
		...
		//Debug.Log(wipedArea.size);

		if (Mathf.Sqrt(wipedArea.width * wipedArea.height) >= 0.8 && isSolved ==false ) {
			//Debug.Log ("wowwwww");
			isSolved = true;
			flowchart.SetBooleanVariable ("hasErased", true);
		}
    ...
    public void enable()
    {
        Camera.main.GetComponent<cameraMasking>().enabled = true;
    }
}
